...
export const registerDriver = async (payload) => {
  ...
  store.dispatch({
    type: driverConstant.REGISTER_DRIVER,
...
export const rideRequest = async (payload) => {
  const add_ride_req = await axios.post(
    "http://localhost:5000/RideRequest",
    payload
  );
  store.dispatch({
    type: cr_ride_ReqCons.CREATE_RIDE_REQ,
    payload: add_ride_req.data,
  });
...
export const getrideRequests = async (payload) => {
  ...
  store.dispatch({
    type: driverConst.GET_ALL_RIDES_REQ,
    payload: get_rides.data,
  });
